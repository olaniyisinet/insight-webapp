...
import './RevenuePortfolioTable.css'
...
const RevenuePortfolio = () => {
  ...
  const asyncFetch = (graphOption) => {
    axios.post(getRequestUrl("/graph-by-name"), {
      ...
      "graphName": "generator-revenue-portfolio",
  ...
  return (
    <div>
      <div className='RevPortfolioHeader'>
        <div className='RevPortfolioTitle'>
          <span className='RevPortfolioTitleHighlight'>Rev. Portfolio</span>
        </div>
        <div className='RevPortfolioOptions'>
          <Button
             onClick={exportData}
      ...
      <div className='RevPortfolio_Box'>

      <List
      dataSource={apiData.graphData}
      renderItem={(item, index)  => (
        <List.Item
          extra={<span style={{fontSize: '20px', fontWeight: '700'}}><span style={{fontSize: '10px',fontWeight: '200'}}>{graphDataUnit} </span>{formatNumber(item.value)} </span>} >
